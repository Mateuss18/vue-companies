# DESENVOLVEDOR(A) FULLSTACK .NET CORE + VUE.JS
## Sobre a vaga
MENU

EN
PT
serviços
cases
sobre
blog
vagas
EN
PT

contato

Desenvolvedor(a) Fullstack .NET Core + Vue.js

Quer fazer parte de um time f\*da?

Somos uma startup que vem crescendo rapidamente e desenvolvendo softwares para clientes nacionais e internacionais. Procuramos alguém que seja inteligente o bastante para entender que nunca sabemos tudo! Sempre temos mais o que aprender. É assim, aprendendo e colaborando, que criamos softwares e experiências únicas! Estamos à procura de profissionais criativos e proativos que queiram desenvolver um trabalho de qualidade na área de desenvolvimento.

Requisitos Necessários

Experiência em .NET Core em produção; APIs;
Experiência com SQL Server e MongoDB;
Experiência com RabbitMQ;
Experiência com ORM (EF/NHibernate);
Experiência com Alta performance e disponibilidade;
Experiência com Kubernetes;
Experiência com GitHub/GitLab;
Experiência com Vue.js;
Boa Comunicação e colaboração em equipe.

Requisitos Diferenciais

Arquitetura orientada a eventos e idempotência;
Observabilidade (logs/métricas/tracing);
CI/CD (GitHub/GitLab);
Segurança em APIs;
Cache e redução de latência;
Padrões arquiteturais (quando aplicável);
Experiência com documentos fiscais eletrônicos
Mentoria (Sênior)

Oferecemos

Excelente ambiente de trabalho para desenvolver suas habilidades;
Ambiente amigável e casual;
Trabalho remoto;
Carga horária flexível;
Um dia de folga no seu aniversário.

Quer fazer parte deste time? Então envie seu currículo para recrutamento@bravi.com.br e em breve entraremos em contato com você!

---

Extraído em: 20/03/2026
Link: https://www.linkedin.com/jobs/view/4372786354/
